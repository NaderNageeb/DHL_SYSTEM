...
$sql = "SELECT * FROM `admin` where `admin_name` = '$username' and `pass` = '$pass'"; 
...
function User_request ($rec_name,$user_id,$service_id,$type,$rec_coun,$rec_add,$rec_phone,$date,$total){
global $conn;
	 $query_check = "SELECT * FROM requests where `service_id` = $service_id 
	and `user_id`= $user_id and `shipp_type`= '$type'
	and `rec_cun` = '$rec_coun' and `rec_add` = '$rec_add'
	 and `rec_phone`= '$rec_phone' and `ship_date` = '{$date}' 
	...
	{
	 echo "<script>alert('Your Shipment Already Exists');</script>";
	//echo '<div class="alert alert-warning bg-primary text-light border-0 alert-dismissible fade in" role="alert" style="text-align:center">Exist</div>';
	 ...
	 $query = "insert into requests(rec_name,user_id,service_id,shipp_type,rec_cun,rec_add,rec_phone,ship_date,total_price)
	 values ('$rec_name',$user_id,$service_id,'$type','$rec_coun','$rec_add','$rec_phone','{$date}','$total')";
	 if(mysqli_query($conn,$query)){
		 
		 echo "<script>alert('Your Shipment Request Submitted Successfully');window.location = './service_detils.php?service_id=$service_id';</script>";
	 }else{
		  echo "<script>alert('Your Shipment Request Faild');</script>";
	 }
	}
...
function Del_req_admin($del_id){
	
	global $conn;

	$sql = "UPDATE requests set `req_del` = 1 where `request_id` = '$del_id'";
if($query = mysqli_query($conn,$sql)){
	echo "<script>alert('Deleted Successfully');window.location = 'shipping_requests.php';</script>";
}else{
	 echo "<script>alert('Deleted Faild');</script>";
}
...
function Approve_notification($approve_id,$user_id,$feedback,$admin_id){
	global $conn;

	$sql = "UPDATE requests set `status` = 1 ,`admin_feedback`='{$feedback}',`admin_id`= $admin_id where `request_id` = $approve_id and `user_id` = $user_id";
	if($query = mysqli_query($conn,$sql)){
	echo "<script>alert('Approved Successfully and Notification Sended');window.location = 'Shipping_requests_detils.php?request_id=$approve_id';</script>";
	// echo "<script>alert('Approved Successfully and Notification Sended');</script>";
...
function Reject_notification($reject_id,$user_id,$feedback,$admin_id){
	global $conn;

	$sql = "UPDATE requests set `status` = 2 ,`admin_feedback`='{$feedback}',`admin_id`= $admin_id where `request_id` = $reject_id and `user_id` = $user_id";
	if($query = mysqli_query($conn,$sql)){
	echo "<script>alert('Rejected Successfully and Notification Sended');window.location = 'shipping_requests.php';</script>";
	}else{
	 echo "<script>alert('Rejected Faild');</script>";
	 echo $sql;
	}
	...
	function Sign($request_id,$col_id){
		global $conn;

		$sql = "UPDATE requests set `collecter_id` = $col_id where `request_id` = $request_id";
		if($query = mysqli_query($conn,$sql)){
		echo "<script>alert(' Asigned Successfully ');window.location = 'shipping_requests.php';</script>";
		}else{
		 echo "<script>alert('Asigned Faild');</script>";
		 echo $sql;
		}
...
if($user_name =="" && $date == "" && $status == "" && $date2 == ""){
	$sql = "SELECT * FROM `users` u,`requests`r , `service`s where r.service_id = s.service_id  and r.`user_id`= u.user_id and r.req_del = 0  ";
	}	

if($user_name !="" && $date == "" && $status == "" && $date2 == ""){
$sql = "SELECT * FROM `users` u,`requests`r , `service`s where r.service_id = s.service_id  and r.`user_id`= u.user_id and u.F_Name LIKE '%$user_name%' and r.req_del = 0  ";
}

if($user_name =="" && $date != "" && $status == "" && $date2 == ""){
	$sql = "SELECT * FROM `users` u,`requests`r , `service`s where r.service_id = s.service_id  and r.`user_id`= u.user_id and r.ship_date = '{$date}' and r.req_del = 0  ";
	}

	if($user_name =="" && $date != "" && $status == "" && $date2 != ""){
		$sql = "SELECT * FROM `users` u,`requests`r , `service`s where r.service_id = s.service_id  and r.`user_id`= u.user_id and r.ship_date BETWEEN '{$date}' and '{$date2}' and r.req_del = 0  ";
		}

		if($user_name =="" && $date == "" && $status == "" && $date2 != ""){
			$sql = "SELECT * FROM `users` u,`requests`r , `service`s where r.service_id = s.service_id  and r.`user_id`= u.user_id and r.ship_date = '{$date2}' and r.req_del = 0  ";
			}



if($user_name =="" && $date == "" && $status != "" && $date2 == ""){
	$sql = "SELECT * FROM `users` u,`requests`r , `service`s where r.service_id = s.service_id  and r.`user_id`= u.user_id and r.status = $status and r.req_del = 0  ";
	}

if($user_name !="" && $date != "" && $status != "" && $date2 == ""){
	$sql = "SELECT * FROM `users` u,`requests`r , `service`s where r.service_id = s.service_id  and r.`user_id`= u.user_id and r.ship_date = '{$date}' and u.F_Name LIKE '%$user_name%' and r.status = $status and r.req_del = 0  ";
	}
...
if($query = mysqli_query($conn,$sql))
	...
	{
		echo "<script>alert('No Data Selected');window.location = 'shipping_reports.php';</script>";
		exit;
	}
...
function Get_requests(){
	global $conn;
 $sql = "SELECT * FROM `users` u,`requests`r , `service`s where r.service_id = s.service_id   and r.`user_id`= u.user_id and  r.collecter_id = 0 and r.status = 0  and r.req_del = 0 order by r.request_id ASC ";
if($query = mysqli_query($conn,$sql))
	{
	return $query;	
	}
...
function Get_requests_app(){
	global $conn;
 $sql = "SELECT * FROM `users` u,`requests`r , `service`s,`collectors`c where c.collecter_id = r.collecter_id and r.service_id = s.service_id   and r.`user_id`= u.user_id and  r.status != 5 and r.collecter_id !=0  and r.req_del = 0 order by r.request_id ASC ";
if($query = mysqli_query($conn,$sql))
	{
	return $query;	
	}
